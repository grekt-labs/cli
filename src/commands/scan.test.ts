...
describe("scan", () => {
  ...
  describe("evaluateFailOn", () => {
    ...
    function makeResult(artifactId: string, badge: TrustBadge, trusted?: boolean): ArtifactScanResult {
      const summary = makeReport(badge)
      return {
        artifactId,
        scanners: [{ scanner: "agentverus", report: summary }],
        summary,
        trusted,
      }
    }
    ...
    test("returns no failure when all badges are below threshold", () => {
      const results = [
        makeResult("@scope/a", "certified"),
        makeResult("@scope/b", "conditional"),
      ]
    ...
    test("returns failure when one badge meets threshold exactly", () => {
      const results = [makeResult("@scope/a", "suspicious")]
      const result = evaluateFailOn(results, "suspicious")
    ...
    test("returns failure when one badge exceeds threshold", () => {
      const results = [makeResult("@scope/a", "rejected")]
      const result = evaluateFailOn(results, "suspicious")
      expect(result.failed).toBe(true)
    ...
    test("excludes trusted artifacts from evaluation", () => {
      const results = [makeResult("@scope/a", "rejected", true)]
      const result = evaluateFailOn(results, "suspicious")
      expect(result.failed).toBe(false)
      expect(result.failedArtifacts).toHaveLength(0)
    })
    ...
    test("does not trigger failure for trusted artifact with bad badge", () => {
      const results = [
        makeResult("@scope/risky", "rejected", true),
        makeResult("@scope/safe", "certified"),
      ]
      const result = evaluateFailOn(results, "suspicious")
      expect(result.failed).toBe(false)
    })
    ...
    test("evaluates only untrusted artifacts in mixed results", () => {
      const results = [
        makeResult("@scope/trusted-bad", "rejected", true),
        makeResult("@scope/untrusted-bad", "suspicious"),
        makeResult("@scope/untrusted-good", "certified"),
      ]
      const result = evaluateFailOn(results, "suspicious")
      expect(result.failed).toBe(true)
      expect(result.failedArtifacts).toHaveLength(1)
